...
export default function CategoryPage({ catId, loggedIn, user, lang, theme, fontSize, setTheme, setFontSize, onLangPick, onLogin, onSignup, onLogout, onHome, onProfile, onAdmin, onChat, bookmarkHook }) {
  ...
  if (!cat) return <div style={{ padding:40,textAlign:'center',color:'var(--text-primary)' }}>Category not found.</div>;
      ...
      <div style={{ maxWidth:740,margin:'0 auto',padding:'24px 20px' }}>
        <div style={{ display:'flex',alignItems:'center',gap:12,marginBottom:8 }}>
          ...
          <div>
            ...
            <div style={{ fontSize:12,color:'var(--text-faint)' }}>{cat.laws.length} laws covered · Click any law for AI explanation</div>
          </div>
        </div>

        <div style={{ marginBottom:20,padding:'10px 14px',background:'var(--accent-bg)',border:'1px solid var(--accent-border)',borderRadius:9,fontSize:12,color:'var(--text-muted)',lineHeight:1.6 }}>
          💡 Each card shows the section number, plain-language description, and what you should do. Click <strong style={{ color:'var(--accent)' }}>Ask AI</strong> for a personalised answer.
        </div>

        {cat.laws.map((law, i) => (
          <div key={i} className="card" style={{ borderColor:'var(--border-light)',marginBottom:9,cursor:'default',animation:`fadeUp 0.3s ease ${i*0.05}s both` }}>
            <div style={{ display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:8,marginBottom:4 }}>
              ...
              <button
                ...
                style={{ background:'none',border:'none',cursor:'pointer',fontSize:18,padding:'0 2px',flexShrink:0,opacity:isBookmarked(`law_${cat.id}_${i}`)?1:0.35,color:isBookmarked(`law_${cat.id}_${i}`)?'var(--accent-gold)':'var(--text-primary)',transition:'all 0.2s' }}>
            ...
            <div style={{ fontSize:12,color:'var(--text-secondary)',lineHeight:1.65,marginBottom:9 }}>{law.desc}</div>
            <div style={{ fontSize:12,color:'var(--green)',background:'var(--green-bg)',border:'1px solid var(--green-border)',borderRadius:7,padding:'7px 11px',marginBottom:9,lineHeight:1.5 }}>✅ {law.actionable}</div>
