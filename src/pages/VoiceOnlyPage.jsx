...
export default function VoiceOnlyPage({ lang, speech, onHome }) {
  ...
  return (
    <div style={{ height: '100vh', background: 'var(--bg-primary)', color: 'var(--text-primary)', fontFamily: 'var(--font-serif)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden' }}>
      {/* Orbits */}
      ...
      {/* Back button */}
      <button onClick={onHome} style={{ position: 'absolute', top: 20, left: 20, background: 'var(--bg-card)', border: '1px solid var(--border-light)', color: 'var(--text-muted)', borderRadius: 8, padding: '8px 14px', cursor: 'pointer', fontSize: 13, fontFamily: 'inherit', zIndex: 10 }}>
        ← Back
      </button>
      ...
      <div style={{ position: 'absolute', top: 20, right: 20, display: 'flex', alignItems: 'center', gap: 8, zIndex: 10 }}>
        ...
        <span style={{ fontSize: 14, fontWeight: 700, background: 'var(--accent-gradient)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>KanoonSaathi</span>
      </div>

      {/* Title */}
      <div style={{ position: 'absolute', top: 28, textAlign: 'center', zIndex: 10 }}>
        <div style={{ fontSize: 15, fontWeight: 700 }}>Voice Mode</div>
        <div style={{ fontSize: 11, color: 'var(--text-faint)' }}>No typing needed — just speak</div>
      </div>
      ...
      <div style={{ marginTop: 24, textAlign: 'center', zIndex: 10 }}>
        ...
        {transcript && status === 'listening' && (
          <div style={{ fontSize: 14, color: 'var(--text-secondary)', maxWidth: 400, lineHeight: 1.6, animation: 'fadeIn 0.2s ease' }}>"{transcript}"</div>
        )}
      ...
      {messages.length > 0 && status === 'idle' && (
        <div style={{ position: 'absolute', bottom: 24, left: 20, right: 20, textAlign: 'center', zIndex: 10 }}>
          <div style={{ fontSize: 10, color: 'var(--text-faint)', marginBottom: 6 }}>Last response:</div>
          <div style={{ fontSize: 12, color: 'var(--text-muted)', lineHeight: 1.5, maxHeight: 60, overflow: 'hidden', textOverflow: 'ellipsis' }}>
