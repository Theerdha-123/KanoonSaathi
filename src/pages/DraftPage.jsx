        ...
        return (
          <div key={i} style={{ display:'flex', alignItems:'center', flex:1 }}>
            <button onClick={() => onStep(i)} style={{
              width:32, height:32, borderRadius:'50%', border: active ? '2px solid var(--accent)' : done ? '2px solid var(--green)' : '2px solid var(--border-medium)',
              background: done ? 'var(--green)' : active ? 'var(--accent-bg)' : 'transparent',
            ...
            <div style={{ flex:1, marginLeft:8 }}>
              <div style={{ fontSize:11, fontWeight: active ? 700 : 400, color: active ? 'var(--accent)' : done ? 'var(--green)' : 'var(--text-faint)' }}>{label}</div>
            </div>
            {i < steps.length - 1 && <div style={{ width:40, height:2, background: done ? 'var(--green)' : 'var(--border-light)', margin:'0 8px', borderRadius:2 }} />}
          </div>
        );
      })}
    </div>
  ...
  return (
    <button onClick={onClick} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}
      style={{
        ...
        background: hover ? 'var(--accent-bg)' : 'var(--bg-card)',
        border: hover ? '1px solid var(--accent-border)' : '1px solid var(--border-light)',
        cursor:'pointer', textAlign:'left', fontFamily:'inherit', color:'var(--text-primary)', transition:'all 0.25s', width:'100%',
      }}>
      <div style={{ display:'flex', alignItems:'center', gap:14, marginBottom:10 }}>
        <div style={{ fontSize:32, width:48, height:48, borderRadius:12, background:'var(--accent-bg)', display:'flex', alignItems:'center', justifyContent:'center' }}>{tpl.icon}</div>
        <div style={{ flex:1 }}>
          <div style={{ fontSize:15, fontWeight:700 }}>{tpl.title}</div>
          <div style={{ fontSize:12, color:'var(--text-muted)', marginTop:2 }}>{tpl.desc}</div>
        </div>
        <span style={{ padding:'3px 10px', borderRadius:12, background:'var(--accent-bg)', border:'1px solid var(--accent-border)', fontSize:10, color:'var(--accent)', fontWeight:600 }}>{tpl.category}</span>
      </div>
...
function DocumentPreview({ draft, templateTitle, templateIcon }) {
  const formatted = draft
    .replace(/\[TO BE FILLED\]/g, '<mark style="background:var(--accent-bg);color:var(--accent);padding:1px 4px;border-radius:3px;font-weight:600">[TO BE FILLED]</mark>')
    .replace(/^(#{1,3})\s+(.+)$/gm, (_, h, t) => {
      const sz = h.length === 1 ? 18 : h.length === 2 ? 15 : 13;
      return `<div style="font-size:${sz}px;font-weight:700;margin:18px 0 8px;color:var(--text-primary);border-bottom:${h.length===1?'1px solid var(--border-light)':'none'};padding-bottom:${h.length===1?'8px':'0'}">${t}</div>`;
    })
  ...
  return (
    <div style={{
      background:'var(--bg-card)', border:'1px solid var(--accent-border)', borderRadius:16,
      padding:'32px 28px', fontFamily:'var(--font-serif)', lineHeight:1.8, fontSize:13.5,
      color:'var(--text-primary)', position:'relative', boxShadow:'0 4px 24px rgba(0,0,0,0.1)',
    }}>
      {/* Letterhead */}
      <div style={{ textAlign:'center', marginBottom:24, paddingBottom:16, borderBottom:'2px solid var(--accent-border)' }}>
        <div style={{ fontSize:14, letterSpacing:1, color:'var(--accent)', fontWeight:700, marginBottom:4 }}>
          {templateIcon} {templateTitle.toUpperCase()}
        </div>
        <div style={{ fontSize:10, color:'var(--text-faint)' }}>Generated by KanoonSaathi AI · For Review Only</div>
      </div>
      <div dangerouslySetInnerHTML={{ __html: formatted }} />
      {/* Footer */}
      <div style={{ marginTop:28, paddingTop:14, borderTop:'1px solid var(--border-light)', fontSize:11, color:'var(--text-muted)', textAlign:'center', lineHeight:1.7 }}>
        ⚠️ AI-generated draft · Review with a qualified advocate before submission<br/>
        Replace all <mark style={{ background:'var(--accent-bg)', color:'var(--accent)', padding:'1px 4px', borderRadius:3, fontSize:10 }}>[TO BE FILLED]</mark> placeholders with actual information
      </div>
    </div>
  );
}
    ...
    <div style={{ minHeight:'100vh', background:'var(--bg-primary)', color:'var(--text-primary)', fontFamily:"'Inter',-apple-system,sans-serif" }}>
      ...
      <div style={{ maxWidth:800, margin:'0 auto', padding:'28px 20px' }}>
        ...
        <div style={{ textAlign:'center', marginBottom:28, animation:'fadeUp 0.3s ease' }}>
          <div style={{ fontSize:48, marginBottom:10, filter:'drop-shadow(0 0 12px var(--accent-border))' }}>📝</div>
          <div style={{ fontSize:24, fontWeight:800, marginBottom:6, background:'var(--accent-gradient)', WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent' }}>AI Legal Draft Generator</div>
          <div style={{ fontSize:13, color:'var(--text-muted)', lineHeight:1.7, maxWidth:500, margin:'0 auto' }}>
            Generate production-ready legal documents — FIR, Legal Notice, RTI, Rent Agreement & more. Updated for BNS/BNSS 2023.
          </div>
          {history.length > 0 && (
            <button onClick={() => setShowHistory(!showHistory)} style={{ marginTop:12, padding:'6px 16px', borderRadius:20, background:'var(--bg-card)', border:'1px solid var(--border-light)', color:'var(--text-muted)', fontSize:11, cursor:'pointer', fontFamily:'inherit' }}>
        ...
        {showHistory && history.length > 0 && (
          <div style={{ marginBottom:24, padding:16, background:'var(--bg-card)', border:'1px solid var(--border-light)', borderRadius:14, animation:'fadeUp 0.2s ease' }}>
            <div style={{ fontSize:12, fontWeight:700, color:'var(--accent)', marginBottom:12 }}>📂 Recent Drafts</div>
            <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
              {history.map(h => (
                <button key={h.id} onClick={() => loadFromHistory(h)} style={{
                  ...
                  cursor:'pointer', fontFamily:'inherit', color:'var(--text-primary)', textAlign:'left', display:'flex', alignItems:'center', gap:10, transition:'all 0.2s',
                }}>
                  <span style={{ fontSize:20 }}>{h.icon}</span>
                  <div style={{ flex:1 }}>
                    <div style={{ fontSize:12, fontWeight:600 }}>{h.title}</div>
                    <div style={{ fontSize:10, color:'var(--text-faint)' }}>{new Date(h.date).toLocaleDateString('en-IN', { day:'numeric', month:'short', year:'numeric', hour:'2-digit', minute:'2-digit' })}</div>
                  </div>
        ...
        {wizardStep === 0 && (
          <div style={{ animation:'fadeUp 0.3s ease' }}>
            <div style={{ fontSize:13, fontWeight:700, color:'var(--accent)', marginBottom:16 }}>Choose a document type:</div>
        ...
        {tpl && wizardStep >= 1 && wizardStep <= totalFormSteps && (
          <div style={{ animation:'fadeUp 0.25s ease' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:20 }}>
              <button onClick={resetAll} style={{ background:'none', border:'1px solid var(--border-light)', borderRadius:8, padding:'6px 14px', color:'var(--text-muted)', cursor:'pointer', fontSize:11, fontFamily:'inherit' }}>← Back</button>
              <div style={{ padding:'5px 14px', borderRadius:20, background:'var(--accent-bg)', border:'1px solid var(--accent-border)', fontSize:12, color:'var(--accent)', fontWeight:600 }}>
                {tpl.icon} {tpl.title}
              </div>
            </div>
            ...
            {/* Auto-fill notice */}
            {user && (
              <div style={{ marginBottom:16, padding:'8px 14px', background:'var(--green-bg)', border:'1px solid var(--green-border)', borderRadius:10, fontSize:11, color:'var(--text-muted)', display:'flex', alignItems:'center', gap:8 }}>
              ...
              return (
                <div>
                  <div style={{ fontSize:16, fontWeight:700, marginBottom:16, display:'flex', alignItems:'center', gap:8 }}>
                    <span style={{ width:28, height:28, borderRadius:8, background:'var(--accent-bg)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:13, color:'var(--accent)', fontWeight:700 }}>{wizardStep}</span>
                    {step.title}
                  </div>
                  {step.fields.map(fieldKey => {
                    ...
                    return (
                      <div key={fieldKey} style={{ marginBottom:18 }}>
                        <label style={{ fontSize:12, color:'var(--text-muted)', display:'block', marginBottom:6, fontWeight:500 }}>{meta.label}</label>
                        {meta.type === 'textarea' ? (
            ...
            {generating && (
              <div style={{ padding:28, textAlign:'center', marginTop:16, animation:'fadeUp 0.3s ease' }}>
                <div style={{ display:'flex', gap:6, justifyContent:'center', marginBottom:14 }}>
                  {[0,1,2,3,4].map(j => <div key={j} style={{ width:8, height:8, borderRadius:'50%', background:'var(--accent)', animation:`pulse 1.4s ${j*0.15}s infinite` }} />)}
                </div>
                <div style={{ fontSize:14, fontWeight:600, color:'var(--text-secondary)', marginBottom:6 }}>Drafting your {tpl.title}…</div>
                <div style={{ fontSize:11, color:'var(--text-faint)' }}>Applying proper legal formatting, BNS/BNSS references, and section headers</div>
              </div>
            )}
          </div>
        )}
        ...
        {tpl && isPreviewStep && draft && (
          <div style={{ animation:'fadeUp 0.3s ease' }} ref={previewRef}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:20, flexWrap:'wrap' }}>
              <button onClick={resetAll} style={{ background:'none', border:'1px solid var(--border-light)', borderRadius:8, padding:'6px 14px', color:'var(--text-muted)', cursor:'pointer', fontSize:11, fontFamily:'inherit' }}>← New Draft</button>
              <div style={{ padding:'5px 14px', borderRadius:20, background:'var(--green-bg)', border:'1px solid var(--green-border)', fontSize:12, color:'var(--green)', fontWeight:600 }}>
                ✅ Draft Generated
              </div>
              <div style={{ flex:1 }} />
              {/* Export buttons */}
              <button onClick={copyDraft} style={{ padding:'7px 14px', borderRadius:8, background:'var(--accent-bg)', border:'1px solid var(--accent-border)', color:'var(--accent)', fontSize:11, cursor:'pointer', fontFamily:'inherit', fontWeight:600 }}>
                {copied ? '✅ Copied!' : '📋 Copy'}
              </button>
              <button onClick={printDraft} style={{ padding:'7px 14px', borderRadius:8, background:'var(--accent-bg)', border:'1px solid var(--accent-border)', color:'var(--accent)', fontSize:11, cursor:'pointer', fontFamily:'inherit', fontWeight:600 }}>
                🖨️ Print/PDF
              </button>
              <button onClick={shareDraft} style={{ padding:'7px 14px', borderRadius:8, background:'var(--green-bg)', border:'1px solid var(--green-border)', color:'var(--green)', fontSize:11, cursor:'pointer', fontFamily:'inherit', fontWeight:600 }}>
                📱 WhatsApp
              </button>
            </div>
            ...
            {/* Legal Disclaimer */}
            <div style={{ marginTop:20, padding:'14px 18px', background:'var(--accent-bg)', border:'1px solid var(--accent-border)', borderRadius:12, fontSize:11, color:'var(--text-muted)', lineHeight:1.7 }}>
              <strong style={{ color:'var(--accent)' }}>⚠️ Legal Disclaimer:</strong> This document is AI-generated using KanoonSaathi and is intended as a starting template only. It does NOT constitute legal advice. Please have this draft reviewed by a qualified advocate or your nearest DLSA (Helpline: 15100) before official submission. All references to BNS 2023, BNSS 2023, and other statutes should be independently verified.
            </div>
